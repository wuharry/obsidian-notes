...
> 更新：2026-09-07｜整理版 v0.4｜原 v0.3 展望內容保留於文末。
> 進度主要依本機 `star-warfare-godot` 工作樹；`star-warfare` 是原 Unity 復原來源，不能把原始碼或素材存在當成 Godot 功能完成。保留原檔名與 `notion-id`。
...
| 武器 | 已實作 | 原武器資料、射擊類型、武器模型與攻擊回饋 | `GameState.WEAPON_ROWS`、`scripts/game/projectile.gd` |
| 彈匣與換彈 | 部分實作 | 指定武器具不同換彈方式、彈匣／彈殼演出，散彈可中斷裝填 | `GameState.RELOAD_PROFILES`、`scripts/game/player.gd` |
| 裝甲／背包／套裝 | 已實作 | 原裝甲的購買、裝備、四部位套裝、背包容量與外觀 | `scripts/core/armor_catalog.gd`、`scripts/core/game_state.gd` |
| 裝甲主動技能 | 已實作 | 原技能效果、持續時間、冷卻與 HUD 顯示 | `scripts/game/armor_power_controller.gd`、`scripts/ui/hud.gd` |
| 商店與軍械庫 | 已實作 | 武器與裝甲分類、解鎖階級、Credits／Mithril 購買、持有狀態與預覽 | `scripts/ui/main_menu.gd`、`scripts/ui/unity_equipment_shell.gd` |
| 補給品商店 | 部分實作 | 已有分類、購買、庫存上限與保存；未找到完整戰鬥使用接線，不能只憑效果資料判定可用 | `scripts/core/props_catalog.gd`、`GameState.purchase_prop()`、`tests/props_store_test.gd` |
| 敵方 AI／難度 | 已實作／體驗待複驗 | 追擊與路徑、戰術難度設定、包抄／攻擊名額／預警等行為 | `scripts/game/enemy.gd`、`GameState.DIFFICULTY_PROFILES` |
| 大陸戰場 | 已實作／體驗待複驗 | 連續地形、原場景地標、隨位置載入與回收、區域遭遇；不同於關卡波次模式 | `scripts/game/expanse_world.gd`、`expanse_terrain.gd`、`sector_landmark.gd` |
| 保存、語言與畫質 | 已實作 | 本機 JSON 保存、繁中／英文、畫質選項 | `scripts/core/game_state.gd`、`scripts/core/localization.gd` |
| 多種輸入與音畫 | 已實作／實機待驗收 | 鍵鼠、控制器、手機雙搖桿；復原音樂音效、角色動畫、HUD | `scripts/ui/virtual_joystick.gd`、`scripts/core/audio_director.gd`、`scripts/game/player.gd` |

### 規模與現行限制

| 項目 | 核對結果 | 不能誤認為 |
|---|---|---|
| 單人關卡 | Level 1–8，共 8 關；`CAMPAIGN_FULLY_UNLOCKED = true` 方便測試，進度仍記錄 | 正式平衡後的解鎖／養成流程已驗收 |
| PvP 地圖 | Level 13–21，共 9 張；入口明示離線預覽，沒有 PvE 波次 | 已能真人連線 PvP |
| 原場景總數 | 17 張關卡資源 | 17 關完整 PvE 戰役 |
| 武器資料 | 47 把，來自 `WEAPON_ROWS` | 47 把已具新配件、模塊與滿級質變 |
| 裝甲 | 21 套 × 4 部位 + 25 背包，共 109 件 | 新展望的品質選槽／商店寶箱雙軌已完成 |
| 武器裝備欄 | 最大 8 格，新增裝備受背包容量限制 | 每個背包都固定能裝 8 把 |
| 裝甲主動技能 | 10 種原技能定義 | 未來企劃「下次再死／動能／腎上腺素」等新套裝方案已全部落地 |
| 補給品 | 11 項：HEALTH 6、AID-KIT 2、ASSIST 3；庫存上限 99 | 全部效果已能在戰鬥中施放 |
| 大陸尺寸 | 3000 × 3000 引擎單位，17 個地標；屬設計尺寸 | 真實地理公里數、量測過的流暢度或完整開放世界任務 |
| 付費 | 使用本機保存的 Credits／Mithril 商店；選單說明原購買服務不在線 | 真實付費、帳號與官方服務已恢復 |

### 已有換彈範圍

| 武器 ID | 換彈類型 | 彈匣容量 | 基礎換彈時間（程式參數） |
|---|---|---|---|
| `gun00` | 步槍彈匣 | 30 | 1.55 秒 |
| `gun35` | 狙擊彈匣 | 5 | 2.18 秒 |
| `gun06` | 散彈逐發裝填 | 6 | 每發 0.68 秒 |
| `gun11` | 火箭裝填 | 1 | 2.42 秒 |
| `gun14` | 榴彈轉鼓 | 4 | 2.05 秒 |

- 目前備彈設為無限；舊 Energy 帳仍保留並在射擊時消耗，是過渡實作。其餘武器不能因上述換彈樣板存在就列為全面完成。
- `README.zh-TW.md` 的「沒有一般彈匣換彈」屬較早描述，與現行 `RELOAD_PROFILES` 及換彈測試不同；此處按實作更新。
...
```text
主選單 → 商店／軍械庫 → 配置武器與裝甲
       ├─ 單人戰役 → 波次／Boss → 掉落與結算 → 本機保存
       ├─ 大陸戰場 → 地標探索／區域遭遇 → 收益保存
       └─ 多人入口 → 原 PvP 地圖離線預覽（尚無真人對戰）
```
...
| 原四件套裝與背包 | 寶箱裝甲選槽、跨等級混搭規則、黑箱背包取得 | 原裝備已實作；新取得與加成規則未實作 |
...
| 方向（對應原 v0.3） | 狀態 | 尚缺內容 |
|---|---|---|
| 手機優先／PC 輔助 | 部分實作／待驗收 | 手機操作已有；仍需裝置手感、效能與 HUD 驗收 |
| 商店／寶箱雙軌武器 | 部分實作 | 只有原商店基礎；寶箱來源、隨機品、品質、重複品處理未完成 |
| 模塊 | 未實作 | Deadly／Overclocked／Pinpoint／Adaptive 等資料、裝配、結算與 UI |
| 配件解鎖樹 | 未實作／待討論 | 槍管、鏡、握把、槍托、第五槽與等級範圍需一致 |
| 武器滿級質變／副模式 | 未實作 | 各類質變池、自選、保存、技能與副模式切換限制 |
| 低階商店槍覺醒 | 未實作 | 滿級條件、可選特性與平衡 |
| 元素與黑武器特殊能力 | 未實作 | 原作特效武器不等於新元素／品質系統；需新增規格與卡池式目錄 |
| PvP 系統與平衡房 | 未實作 | 真正連線、等級區間配對、標準裝備、房間數值標示 |
| PvP／PvE 分開平衡 | 未實作 | 獨立曲線、禁用副模式、個別技能倍率與冷卻 |
| 寶箱裝甲與新套裝加成 | 未實作 | 品質選槽、低中高階加成選擇、混搭規則；勿與原復原套裝混算 |
| 黑箱專屬背包 | 未實作／待討論 | 現有背包可由商店購買，未改成黑箱專屬；需決定適用新背包或全部背包 |
| 寶箱／課金經濟 | 未實作 | 掉率、鑰匙、付費取得、保存與免費／付費等強度限制 |
| 全域 DPS 上限 | 未實作／待討論 | 基準、傷害公式、品質／模塊疊加及上限執行方式尚未統一 |
| 主線故事 | 未完成 | 原場景復原不等於故事章節與世界觀已完成 |

## 必須先討論的原案衝突

> 本次保留原值與方向，不代替產品做平衡決策。下列是文件算式核對及規格比對，並非遊戲實測。

| 衝突 | 原案證據／具體例子 | 建議處理（尚未定案） |
|---|---|---|
| 終局表與乘法公式不一致 | 用原表近似值 `1.33 × 1.72 × 1.4 = 3.20264`，已超過寫定上限 `2.3x`，也高於黑箱表的約 `2.20x` | 先定義基準與品質是否已包含在終局數值，再統一重算；不悄悄修改任一倍率 |
| 「Soft Cap」與絕對上限混用 | 原文說任何情況不得超過 `2.3x`；若真是不准超過，代表硬性封頂，與超過後只是收益變少的軟上限不同 | 選擇硬上限、遞減收益或純設計目標，再設計實作 |
| 等級表與配件樹範圍不同 | 成長表列至 `Lv.8`，配件樹卻在 `Lv.12` 開槍托 | 確认是否不同系統的等級；若同一武器等級就統一表格 |
| 不同屬性的百分比不能直接相加成 DPS | 傷害 `+12%` 與射速 `+10%` 同時生效，未計其他因素時為 `1.12 × 1.10 = 1.232`，不是 `1.22` | 同屬性內可加總，傷害／射速／爆擊分開代入公式；再定義換彈是否計入 |
| DPS 提升值沒有量測依據 | 紅點鏡「有效 DPS +8%」、滿配「1.25–1.45x」等未附命中率、距離或測試條件 | 視作原案估算／目標，不當作測量結果或平衡通過 |
| 背包新舊取得規則不同 | Godot 已有商店背包，展望寫背包為黑箱專屬 | 決定只對新增背包生效，或重做整個取得方式 |
| 電漿 PvP 表引用未定義效果 | PvP 表寫鏈電目標數減半、地雷最多 1 顆；質變池未完整定義對應目標數與地雷項目 | 先補齊效果或標為保留項，不直接拿來驗收 |

## 建議後續順序（供討論）

| 順序 | 工作 | 驗收方向 |
|---|---|---|
| 先穩定目前底盤 | 射擊、換彈、裝備、存檔、手機操作與現有場景 | 跑對應既有測試，再看實機畫面與操作 |
| 統一新成長規格 | 武器等級、配件槽、DPS 基準與上限 | 用同一套公式重算原表，消除上述矛盾 |
| 做一組完整新裝備流程 | 取得 → 裝配／升級 → 戰鬥生效 → 保存 | 先完成一個可驗收例子，再擴展各類武器 |
| 擴充寶箱與裝甲 | 品質、模塊、選槽、混搭與經濟 | 明確區分新系統與原復原裝備 |
| 完成真人連線再驗 PvP | 配對、對戰結算、PvP 獨立數值 | 離線場景預覽不可當完成證據 |
