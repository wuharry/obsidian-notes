...
> [!abstract] **文件更新與核對說明**
> - **更新日期**：2026-09-17 ｜ 整理版 v2.3 ｜ 依本機 `card-game-demo-4.5--main` 工作樹核對，並補充可玩性與商業 CCG 體驗差距診斷。
> - **結構規範**：上半部描述目前實作、最新待辦與體驗診斷，下半部保存原 v2.0 設計（原設計非完成清單）。保留原檔名與 `notion-id`。
...
> - [[#六、可玩性與對局深度診斷（對齊商業 CCG 差距）|六、可玩性診斷]]
> - [[#七、本次核對與後續交接|七、核對與交接]]
> - [[#八、原 v2.0 完整企劃案（設計保留區）|八、原設計保留區]]
...
| **魔力與回收** | 🟢 已實作    | 成長回滿、費用檢查、拖牌至墓地／回收區回魔與隔回合冷卻        | `BattleManager.apply_discard_for_mana()`、`src/mana_recycle/mana_recycle.gd` |
...
| **操作與展示** | 🟢 已實作    | 拖曳、懸停預覽、扇形手牌、秘術施放區/目標指向/魔力回收拖曳、戰鬥紀錄抽屜介面 (BattleArchive)、3D打擊與秘術高清演出 (VFX)、牌庫圖鑑、傷害治療回饋 | `src/card_manager/card_manager.gd`、`src/battle_ui/battle_ui.gd`、`src/battle_ui/battle_archive.gd`、`docs/battle_archive.md` |
| **墓地內容查閱** | 🟢 已實作    | 點擊墓地地標開啟唯讀視窗，重複卡分開列出、hover 顯示詳情且具備操作防穿透 | `src/battle_ui/battle_archive.gd`、`docs/battle_archive.md` |
| **素材與資源** | 🟢 已實作    | 8.13.20 與 12.26.19 圖示包、像素動畫與特效素材包完整收錄於 repo | `assets/icons_*.zip`、`assets/pixel_animations_gfxpack.zip` |
...
| 卡片類型 | 實際資源數 | 目前定位與功能 |
| :--- | :--- | :--- |
| **從者 (MINION)** | 78 張 | 場上戰鬥、主動技、戰吼與被動（含 Time Fantasy 12 張） |
...
| **總計** | **132 張** | 依 `data/cards/*.tres` 統計；`card_pool_verify` 315 項檢查全過 |
...
- **全域／無目標秘術**：拖出扇形手牌並進入戰場上半部中央的「秘術施放區 (`SpellDropZone`)」時高亮提示，放開即執行施放。
- **目標指向秘術**：拖曳時拉出指向箭頭，懸停在合法從者或英雄目標上方時箭頭與目標高亮，放開執行對該目標的秘術效果；排除拖曳中卡牌自身 Area3D 射線碰撞。
- **魔力回收區 (`ManaRecycle`)**：卡牌拖曳至右側回收區會觸發高亮提示，放開後依規則折半轉換為暫時魔力並入墓；未達成回收條件或放開在無效區域時自動滑順歸位回手牌。
- **手牌防遮擋**：懸停選取的卡片將臨時提升 `render_priority` 並關閉 `depth_test`，避免相鄰手牌相互遮蔽。

### 5. 戰鬥紀錄系統 (Battle Archive)

- **入口與介面**：戰鬥畫面左上方提供「戰鬥紀錄」切換按鈕，可隨時展開或收合全功能抽屜面板 (`src/battle_ui/battle_archive.gd`)。
- **紀錄呈現**：
  - 依照對局回合分類，提供回合分割標籤與時間戳記。
  - 支援行動類型標籤（出牌、攻擊、技能、秘術、回魔、受到傷害）。
  - 支援中英文多語系動態切換（依據 `AppSettings`）。
  - 開啟抽屜時具備操作防穿透判定，防止誤觸牌桌背後卡牌。
...
| **墓地內容查閱** | 🟢 已實作 | 點擊墓地地標可開啟唯讀視窗 (`battle_archive.gd`)，重複卡分開列出、hover 顯示詳情且具備操作防穿透 | 2026-09-12 已驗收，全功能正常運作 |
| **抽卡／卡包系統** | 未實作／暫不需要 | 原企劃含口袋經濟與開卡包；目前聚焦核心對局，戰鬥驗證前不引入盲盒抽卡 | 階段性排除，核心手感 80 分後再議 |
| **後手平衡機制** | 未實作 | 後手魔力落後且無補償，先手勝率過高；需引入 0 費魔力幣或起手多抽 1 張 | 列入 P0 待辦清單實作 |
...
## 六、可玩性與對局深度診斷（對齊商業 CCG 差距）

目前原型具備完整的 5×2 前後排、路線阻擋打臉、丟牌回魔與瞬咒反制等底層規則，但「可玩性、手感爽度與策略反饋」仍與商業 CCG（爐石傳說、闇影詩章 2）有明顯差距。

### 1. 牌組體系化與套路配合（解決隨機大雜燴）
- **痛點**：目前由 132 卡池隨機抽 60 張混編，玩家無法依策略組牌，抽到 10 費高階秘術卻無前期濾牌或回魔元件配合，只能卡死在手牌。
- **解法**：建立兩副互克的主題套牌：
  - **快攻灼燒流**：低費衝鋒怪 + 猛毒瘴氣 + 火焰爆裂，5 回合內突破或繞過前排直取本體。
  - **死靈召喚流**：死靈法師 + 鐵壁肉盾 + 亡軍之門 + 靈魂豐收，拖到後期一回合拉滿場大怪反打。

### 2. 戰鬥打擊感與手感重量 (Game Feel & Juice)
- **痛點**：從者攻擊時原地播放幀動畫、目標頭上飄出數字，缺少實體肉搏碰撞力，整體體驗偏向「數值試算表」。
- **解法**：卡牌攻擊加入向目標位移拉扯衝撞（Tween）、命中短暫停頓（Hit Stop）、受擊卡牌受力晃動、致死重擊鏡頭微震與打擊重音。

### 3. 資源流動防枯竭（防止中後期「抽一打一」垃圾時間）
- **痛點**：每回合只抽 1 張牌且缺乏低門檻過牌手段，「丟牌回魔」還要倒扣手牌並有隔回合冷卻，打到第 5–6 回合雙方手牌耗盡，退化成抽什麼下什麼的純運氣死局。
- **解法**：加入「入場抽牌」、「亡語過牌」或回合開始的「二選一濾牌」，手牌維持 3–5 張以保留決策空間。

### 4. 5×2 卡槽空間防死鎖（避免肉盾爛泥巴仗）
- **痛點**：單位入場後永久釘死在格子上，前排若被雙方高血肉盾（如 1 攻 8 血帶鐵壁）堵住，同路後排大怪既無法攻擊對手本體也無法前進，形成死局。
- **解法**：加入「主動犧牲／卡槽回收」指令，或允許消耗少許魔力在「同路前後排換位」，釋放攻擊路徑。

### 5. 後手補償機制缺失（平抑先手滾雪球優勢）
- **痛點**：先手永遠先召喚、先攻擊、先到達高費；後手魔力與場面雙重落後且無任何補償，勝負在擲骰/猜拳時就決定大半。
- **解法**：後手開局多抽 1 張牌，並給予 1 張一次性「0 費暫時魔力幣」，彌補後手被動解場的劣勢。

### 6. 數值線性對稱打破（缺乏超模神抽刺激感）
- **痛點**：卡牌效果嚴格遵循「N 費做 N 點數值」，缺乏商業 CCG 透過前期鋪陳引爆的「費用魔術 (Mana Cheat)」與全場翻盤 (Swing Turn)。
...
### 8. 抽卡系統的階段性定位（Demo 階段排除）
- **結論**：目前 Demo 階段完全不需要做開卡包或抽卡系統。
- **原因**：抽卡的快感來自「拿到這張卡我能在戰場上打出什麼酷炫套路」，若戰鬥本身打擊感與流派未成熟，抽卡只會淪為開盲盒；且抽卡系統需搭配機率、分解合成與經濟帳本，會嚴重分散打磨核心戰鬥的精力。
- **推進順序**：核心戰鬥手感與流派 (現在) → Demo 固定套牌與教學 → 牌組自訂編輯器 → 最後才做開包與抽卡。
...
| 檢查項目 | 結果與限制說明 |
| :--- | :--- |
| **靜態核對** | 工作樹、玩法規格、卡片資料、操作流程、戰鬥紀錄與可玩性診斷已完成核對 |
| **卡池統計** | 完整核對 132 張資源與分類（78 從者 / 5 靈裝 / 33 秘術 / 8 瞬咒 / 8 伏印） |
| **既有測試入口** | `card_pool_verify.gd` (315 PASS)、`high_tier_cards_verify.gd` (PASS)、`generated_minions_verify.gd` (PASS)、`battle_archive_test.gd` (PASS) |
| **本次執行限制** | 單元測試與架構診斷完成；真機連續 20 局真人手感試玩與 Windows 匯出持續進行中 |
| **下次更新指引** | 先查 `README.md` Gameplay Spec 與 `docs/demo_readiness.md`，再查 `src/`、`data/cards/` 與對應測試 |
