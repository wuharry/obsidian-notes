...
> [!abstract] **文件更新與核對說明**
> - **更新日期**：2026-09-12 ｜ 整理版 v2.2 ｜ 依本機 `card-game-demo(4.5)` 工作樹核對。
...
| **墓地內容查閱** | 🟡 部分實作 | 墓地地標已具備獨立檢視碰撞層 (INSPECT_LAYER: 32) 與堆疊張數顯示，完整墓地彈窗視窗已完成原型審查 (docs/grave_dialog_review.png) | 繼續整合至正式對戰流程 |
...
| 檢查項目 | 結果與限制說明 |
| :--- | :--- |
| **靜態核對** | 工作樹、玩法規格、卡片資料、操作流程與戰鬥紀錄已完成核對（commit `9bc9101e`、`6c55acfb`、`45d734ee`） |
| **卡池統計** | 重新核對 120 張資源與分類（66 從者 / 5 靈裝 / 33 秘術 / 8 瞬咒 / 8 伏印） |
| **既有測試入口** | `tests/arcana_drag_input_test.gd`、`tests/battle_archive_test.gd`、`tests/arcana_drag_capture.gd`、`card_pool_verify.gd`、`ai_strategy_test.gd` |
| **本次執行限制** | 單元測試與截圖已完成；真機手感與網路對戰持續驗證中 |
| **下次更新指引** | 先查 `README.md` Gameplay Spec 與 `docs/battle_archive.md`，再查 `src/`、`data/cards/` 與對應測試 |
