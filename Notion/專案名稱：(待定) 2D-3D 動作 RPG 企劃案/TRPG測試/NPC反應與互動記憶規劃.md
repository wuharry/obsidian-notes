...
- NPC 的合法反應取代該 NPC 本回合舊的自動攻擊，避免求助時仍多打一拳。沒有合法反應提案時先尊重已有的停手記憶，其餘沿用原有敵方回合規則。
...
## 2026-09-12：NPC 動機與怪物習性已接入

NPC 與怪物現在共用既有 DM 反應階段，由 AI 根據作者設定與記憶選擇反應，引擎驗證後保存。未增加獨立 NPC AI 呼叫。

| 項目 | 已實作 |
| --- | --- |
| NPC 動機 | 先補 10 位常見人物的重視、厭惡、恐懼與初始需求；其餘沿用既有性格 |
| 怪物生態 | 洞穴巨蜘蛛、井口蜘蛛、高樑蜘蛛、蠕蟲之母，共 4 個詳細配置；包含棲地、食性、感官、群體關係、威脅反應、追擊界線 |
| 反應能力 | 有戰鬥身體及習性設定的在場怪物可使用 `npcReactions`，`npcId` 填怪物物件 ID；門、蛛網與無形危險不適用 |
| 持續態度 | 每對角色新增 `combatState`，記錄場景與敵對／警戒／退開；停手後不會因原本敵對設定而下一輪自動重開戰 |
| 再次衝突 | 物理攻擊、目標型攻擊法術或新驚動可重啟敵意；多人已指定目標的怪物被另一人攻擊，會保存新的衝突對象 |
| 敘事限制 | 蜘蛛以動作或聲響表達，不能突然說人話；固定在石室的蠕蟲之母不能逃跑 |
| 相容性 | 舊存檔可讀，訪客合併／重載保留新狀態；多人公開串流與重播不公開私人關係記憶 |

```text
玩家行動 + 性格／動機／習性 + 此玩家的互動記憶
  → 同一次 DM 提出條件式反應
  → 玩家行動結算 → 引擎核對在場、傷勢與合法資源
  → 反擊／停手與事件 → 保存 → 敘事 → 下一回合讀回
```

例如蜘蛛可在入侵者退開後停止攻擊，但仍留在磨坊、保持警戒。玩家再次攻擊會重啟敵意；停手不等於馴服，也不自動完成委託。蠕蟲之母的撤退提案則會因固定身體而遭引擎拒絕。

自動測試已覆蓋取消蓄力、重新載入、再次物理／法術攻擊、倒下不能反應與多人隔離。真模型選擇及真人演出仍待驗收；完整生態數量模擬、餵食消耗、馴服、每日遷移與跨場追擊尚未實作。既有記憶容量上限仍適用，超額不保證保留新停手記憶。

- [x] 動機／習性 context、同次 DM、反應驗證、持續態度、訪客存檔與多人公開投影。
- [ ] 真模型實玩蜘蛛領域反應、退開、再攻擊，以及老闆娘忙碌、被打擾與呼援。
- [ ] 其餘 NPC 的個別動機與其他章節詳細生態，依實玩需要補齊。

完整工程紀錄與當次檢查結果：`E:/ai-wrapper/docs/reports/npc-ecology-2026-09-12.md`。專案 TODO 與設計文件也已同步更新。
